...
## read data ##
setwd("/Users/YaqingXie/Desktop/ADS_project5")
train_genre <- read.csv("train_genre.csv")
#train_features <- read.csv("train_features.csv")
#train_id <- train_features[1]
#train_features <- train_features[4:nrow(train_features),2:ncol(train_features)]
#save(train_id, file="train_id.rdata")
load("train_id.rdata")
test_features <- read.csv("test_features.csv")
test_id <- test_features[1]
test_features <- test_features[4:nrow(test_features),2:ncol(test_features)]
...
## knn - train model ##
knn_predict <- function(k, train, test, label){
  knn_model <- knn(train, test, label, k, algorithm="cover_tree")
  indices <- attr(knn_model, "nn.index")
  n <- 0
  for(i in c(1:100)){
    id <- as.character(subset_test_id[i])
    neighbors_k <- indices[i,]
    neighbor_genres <- vector()
    neighbors_ids <- vector()
    for(j in c(1:k)){
      neighbor <- neighbors_k[j]
      neighbor_id <- as.character(subset_train_id[neighbor]) 
...
#  
#  accuracy_12 <- knn_predict(12, subset_train, subset_test, subset_train_id)
#  accuracy_13 <- knn_predict(13, subset_train, subset_test, subset_train_id)
#  accuracy_14 <- knn_predict(14, subset_train, subset_test, subset_train_id)
#  accuracy_15 <- knn_predict(15, subset_train, subset_test, subset_train_id)
#  accuracy_16 <- knn_predict(16, subset_train, subset_test, subset_train_id)
#  accuracy_17 <- knn_predict(17, subset_train, subset_test, subset_train_id)
#  accuracy_18 <- knn_predict(18, subset_train, subset_test, subset_train_id)
#  accuracy_19 <- knn_predict(19, subset_train, subset_test, subset_train_id)
#  accuracy_20 <- knn_predict(20, subset_train, subset_test, subset_train_id)
#  
#  return(c(accuracy_12[[1]], accuracy_13[[1]], accuracy_14[[1]], accuracy_15[[1]], accuracy_16[[1]], accuracy_17[[1]]))
#}
...
test_predict <- knn_predict(k, train_features_pca, test_features_pca, train_id)
test_genres <- test_predict[[2]]
save(test_genres, file = "test_genres.rdata")
test_neighbors <- test_predict[[3]]
save(test_neighbors, file="test_neighbors.rdata")